...
// This function sets a new amplitude value for vibrato.	//V
void MAGE::Mage::setvibamp( double vibamp )
{
	this->vocoder->setvibamp( vibamp );
}

//	This function should access the sine wave used in vocoder for vibrato.  //V
void MAGE::Mage::setsinresult( double sinresult )
{
	this->vocoder->setsinresult ( sinresult );
	return;
}
...
// Sinus wave for vibrato. // V
double MAGE::Mage::mysin ( double indx )
{
	return sin( indx );
}
...
double MAGE::Mage::popSamples ( void )
{
	...
	{
		this->sinusoid++;
		if (this->sinusoid > this->vibthresh)
		{this->sinusoid = 0 ;} // V
		
		this->setsinresult( (double)mysin( (2 * 3.14159265359 * ((double)this->sinusoid/((double)this->vibthresh)) * ((double)this->vibzoom)) + (double)this->viboffset ) ); // for vibrato // V
...
{	
	...
	this->vibthresh = defaultSamplingRate; // V
	this->viboffset = 0;
	this->vibzoom = 1;
